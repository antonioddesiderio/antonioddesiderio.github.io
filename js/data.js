export const DATA = {
    welcome: `
        <p>I am a Postdoctoral Researcher at the <strong>ISI Foundation</strong>, working within the <strong>Complex Connections Lab</strong> (Northeastern University London).</p>
        <p>With a background in physics, I study the digital and physical traces we leave behind to quantitatively understand human dynamics. My research draws on diverse datasets and methods from the Physics of Complex Systems, within the broader framework of Computational Social Science.</p>
        <p>In the digital sphere, I investigate how online coordination can trigger financial events and shape news consumption. In the physical world, I explore how urban design influences human mobility and food networks.</p>
        <p class="welcome-contact">If you have a question or find something interesting here, feel free to get in touch.<br><span class="welcome-contact-sub">I usually reply quickly.</span></p>
    `,
    jobs: [
        {
            title: "Postdoctoral Researcher",
            institution: "ISI Foundation",
            year: "Aug. 2025 - Currently",
            description: `I am a postdoctoral researcher in Urban Dynamics at the ISI Foundation in the <a href="https://www.riccardodiclemente.com" target="_blank">Complex Connections Lab</a>, led by prof. Riccardo Di Clemente.<br>
            I work on the COLINE (COmplex LInks of NEighbourhoods) DUT European Partnership project F-DUT-2023-0037.<br>
            More information can be found here: <a href="https://www.dut-coline.org" target="_blank">COLINE</a>`
        },
        {
            title: "Postdoctoral Researcher",
            institution: "Technical University of Denmark (DTU Compute)",
            year: "Jan. 2024 - Aug. 2025",
            description: `I was a postdoctoral researcher in Urban Multimodal Mobility Modeling at the Technical University of Denmark in the <a href="https://github.com/SocialComplexityLab" target="_blank">Social Complexity Lab</a>, led by Sune Lehmann and Laura Alessandretti.<br>
            I have worked on the GEMINI (Greening European Mobility through cascading innovation INItiatives) project funded by the European Union’s Horizon Innovation Actions programme under Grant agreement No. 10110380.<br>
            More information can be found here: <a href="https://www.geminiproject.eu" target="_blank">GEMINI</a>`
        },
        {
            title: "Ph.D. & Doctor Europaeus in Physics",
            institution: "University of Rome Tor Vergata and Enrico Fermi Research Center",
            year: "Oct. 2020 - Oct 2023",
            description: `Final Grade: excellent quality cum laude<br>
            Thesis Title: Complex dynamics in social systems during recent exogenous and endogenous events<br>
            Topics: Physics, Complex Networks, Epidemic Spreading, Opinion Dynamics, Online Social Media<br>
            Supervisors: Professor <a href="https://www.giuliocimini.com" target="_blank">Giulio Cimini</a>`
        },
        {
            title: "Data Scientist",
            institution: "Bank of Italy",
            year: "Oct. 2019 - Apr. 2020",
            description: `I have built and analysed the Knowledge Graph of Italian Companies (KG-IC), employed during the COVID-19 crisis (Golden Power).<br>
            More informations can be found here: <a href="https://www.bankit.art" target="_blank">ART</a>, <a href="https://kg19.bankit.art" target="_blank">KG-COVID-19</a>`
        },
        {
            title: "Master Degree in Theoretical Physics",
            institution: "University of Rome Tor Vergata",
            year: "Dec. 2016 - May 2019",
            description: `Final Grade: 110/110 cum laude<br>
            Thesis Title: Lattice calculation of the radiative leptonic decay rates of pseudoscalar mesons<br>
            Topics: Physics, Quantum Field Theory, High Performance Computing<br>
            Supervisors: Professor Nazario Tantalo, Dr Martin Hansen`
        },
        {
            title: "Bachelor Degree in Physics",
            institution: "University of Rome Tor Vergata",
            year: "Oct. 2013 - Dec. 2016",
            description: `Final Grade: 110/110 cum laude<br>
            Thesis Title: Formal theory of Scattering: the case of Coulomb potential<br>
            Topics: Physics,Quantum Field Theory<br>
            Supervisors: Professor Nazario Tantalo`
        }
    ],
    contacts: {
        emails: [
            "antonio [dot] desiderio [at] isi [dot] it",
            "antonio [dot] desiderio [at] cref [dot] it",
            "antde [at] dtu [dot] dk"
        ],
        socials: [
            { name: "LinkedIn", url: "https://www.linkedin.com/in/antonio-desiderio-05121994/" },
            { name: "X", url: "https://x.com/ADTony_" },
            { name: "Bluesky", url: "https://bsky.app/profile/adtony.bsky.social" },
            { name: "GitHub", url: "https://github.com/RiegelGestr" },
            { name: "Google Scholar", url: "https://scholar.google.com/citations?user=v3MYshMAAAAJ&hl=it&authuser=1" },
            { name: "ORCID", url: "https://orcid.org/0000-0002-9971-450X" },

        ]
    },
    students: [
        {
            id: 1,
            name: "Anna Mancini",
            collaborators: "Prof. Giulio Cimini, Prof. Riccardo Di Clemente",
            university: "University of Tor Vergata",
            year: "2021",
            tags: ["Master", "Physics", "Social Media"]
        },
        {
            id: 2,
            name: "Nawaf Alaqeili",
            collaborators: "Prof. Riccardo Di Clemente",
            university: "University of Exeter",
            year: "2022",
            tags: ["Master", "Computer Science", "Social Media"]
        },
        {
            id: 3,
            name: "Suhitha Yerabolu",
            collaborators: "Prof. Riccardo Di Clemente",
            university: "University of Exeter",
            year: "2022",
            tags: ["Bachelor", "Computer Science", "Social Media"]
        },
        {
            id: 4,
            name: "Gioacchino Mauri",
            collaborators: "Prof. Giulio Cimini",
            university: "Univeristy of Rome Tor Vergata",
            year: "2022",
            tags: ["Master", "Physics", "Finance"]
        },
        {
            id: 5,
            name: "Giulia Di Palma",
            collaborators: "Prof. Giulio Cimini, Prof. Mauro Sbragaglia, Dr. Francesca Pelusi",
            university: "Univeristy of Rome Tor Vergata",
            year: "2022",
            tags: ["Master", "Physics", "Machine Learning"]
        },
        {
            id: 6,
            name: "Daniele Cirulli",
            collaborators: "Prof. Giulio Cimini, Dr. Fabio Saracco",
            university: "Univeristy of Rome Tor Vergata",
            year: "2023",
            tags: ["Master", "Physics", "Social Media"]
        },
        {
            id: 7,
            name: "Franziska Bäuerlein",
            collaborators: "Prof. Sune Lehmann",
            university: "Technical University of Denmark",
            year: "2024",
            tags: ["Master", "Computer Science", "Food"]
        },
        {
            id: 8,
            name: "Regin Steffansson Kunoy",
            collaborators: "Prof. Sune Lehmann",
            university: "Technical University of Denmark",
            year: "2024",
            tags: ["Master", "Computer Science", "Urban Dynamics"]
        },
        {
            id: 9,
            name: "Diego Rodríguez Gordo",
            collaborators: "Prof. Georgios Arvanitidis",
            university: "Technical University of Denmark",
            year: "2024",
            tags: ["Master", "Computer Science", "Finance"]
        },
        {
            id: 10,
            name: "Vittorio Turco",
            collaborators: "Prof. Vito Latora, Prof. Sune Lehmann, Prof. Laura Alessandretti",
            university: "Scuola Superiore di Catania",
            year: "2025",
            tags: ["Bachelor", "Physics", "Human Behavior"]
        },
        {
            id: 11,
            name: "Margherita Bencini",
            collaborators: "Prof. Daniele Marazzina (polimi), Prof. Paolo Barucca (UCL)",
            university: "Polytechnic University of Milan",
            year: "2025",
            tags: ["Master", "Finance", "Social Media"]
        },
        {
            id: 12,
            name: "Leonardo Rodovero",
            collaborators: "Prof. Sune Lehmann",
            university: "Technical University of Denmark",
            year: "2025",
            tags: ["Master", "Computer Science", "Food"]
        },


    ],
    collaborators: [
        {
            name: "Prof. Sune Lehmann",
            university: "DTU Compute",
            coords: [55.6852823,12.5708177],
            site: "https://sunelehmann.com/",
            tag: "Food"
        },
        {
            name: "Prof. Laura Alessandretti",
            university: "DTU Compute",
            coords: [55.7838539,12.5135907],
            site: "https://laura.alessandretti.com/",
            tag: "Human Mobility"
        },
        {
            name: "Prof. Riccardo Di Clemente",
            university: "Northeastern University London",
            coords: [51.5081157,-0.0779127],
            site: "https://www.riccardodiclemente.com",
            tag: "Urban Dynamics"
        },
        {
            name: "Prof. Giulio Cimini",
            university: "University of Rome Tor Vergata",
            coords: [41.8527643,12.6012077],
            site: "https://www.giuliocimini.com",
            tag: "Physics"
        },
        {
            name: "Dr. Fabio Saracco",
            university: "Enrico Fermi Research Center",
            coords: [41.8998819,12.4917146],
            site: "https://sites.google.com/cref.it/fabiosaracco/",
            tag: "Social Media"
        },
        {
            name: "Dr. Yuan Liao",
            university: "Lund University",
            coords: [55.7119512,13.1986221],
            site: "https://yuan-liao.com/",
            tag: "Human Mobility"
        },
        {
            name: "Prof. Iacopo Iacopini",
            university: "Northeastern University London",
            coords: [51.5098927,-0.0809041],
            site: "https://iaciac.github.io/",
            tag: "Complex Systems"
        },
        {
            name: "Prof. Paolo Barucca",
            university: "University College London",
            coords: [51.5242614,-0.1368477],
            site: "https://paolobarucca.com/",
            tag: "Finance"
        },
        {
            name: "Prof. Luca Pappalardo",
            university: "ISTI-CNR and Scuola Normale Superiore",
            coords: [43.7180897,10.403158],
            site: "https://kdd.isti.cnr.it/people/pappalardo-luca/",
            tag: "Human Mobility"
        },
        {
            name: "Dr. Mattia Mazzoli",
            university: "ISI Foundation",
            coords: [45.0618703,7.6845718],
            site: "https://mattiamazzoli.github.io/",
            tag: "Human Mobility"
        },
        {
            name: "Dr. Sandor Juhasz",
            university: "Corvinus University of Budapest",
            coords: [47.486157,19.0556806],
            site: "https://sandorjuhasz.com//",
            tag: "Urban Dynamics"
        },
        {
            name: "Dr. Laura Silva",
            university: "University of Lausanne",
            coords: [46.5128106,6.5729342],
            site: "https://sites.google.com/view/laura-elinor-silva/home/",
            tag: "Social Science"
        },
    ],
    grants: [
        {
            year: 2025,
            title: "Travel Grant",
            funder: "Otto Moensted Foundation",
            details: "Grant ID 25-81-1979 (2.000 DKK)"
        },
        {
            year: 2024,
            title: "Visiting Grant",
            funder: "SoBigData++ Transnational Access",
            details: "The impact of urban environments on navigation strategies (5.000 EU)"
        },
        {
            year: 2023,
            title: "Visiting Grant",
            funder: "Danish Data Science Academy",
            details: "Grant ID DDSA-V-2023-008 (15.000 DKK)"
        }
    ],
    visiting: [
        {
            period: "October-December 2024",
            institution: "ISTI Institute of CNR, Department of Computer Science Univeristy of Pisa and Scuola Normale Superiore (Pisa)",
            lab: "Knowledge Discovery and Data Mining Laboratory, Mobility Team",
            host: "Luca Pappalardo"
        },
        {
            period: "May-July 2023",
            institution: "DTU Compute, Technical University of Denmark (Copenhagen)",
            lab: "Section for Cognitive Systems, Social Complexity Lab",
            host: "Laura Alessandretti"
        }
    ]
};
